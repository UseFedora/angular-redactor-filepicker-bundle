      ...
      return {
        ...
        html: function (html, clean) {
          ...
          if (this.utils.browser('msie')) {
            ...
            if (this.clean.singleLine) this.insert.execHtml(html);else document.execCommand('insertHTML', false, html);
...
$.Redactor.prototype.fullscreen = () => ({
  ...
  enable: () => {
    _this.fullscreen.isOpened = false;
    _this.button.setActive('fullscreen');
    _this.fullscreen.isOpen = true;

    if (!_this.opts.fullscreen) {
      _this.selection.save();
    }

    if (_this.opts.toolbarExternal) {
      _this.fullscreen.toolcss = {};
      _this.fullscreen.boxcss = {};
      _this.fullscreen.toolcss.width = _this.$toolbar.css('width');
      _this.fullscreen.toolcss.top = _this.$toolbar.css('top');
      _this.fullscreen.toolcss.position = _this.$toolbar.css('position');
      _this.fullscreen.boxcss.top = _this.$box.css('top');
    }

    _this.fullscreen.height = _this.core.editor().height();

    if (_this.opts.maxHeight) {
      _this.core.editor().css('max-height', '');
    }

    if (_this.opts.minHeight) {
      _this.core.editor().css('min-height', '');
    }

    if (!_this.$fullscreenPlaceholder) {
      _this.$fullscreenPlaceholder = $('<div/>');
    }

    _this.$fullscreenPlaceholder.insertAfter(_this.$box);

    _this.$box.appendTo(document.body);

    _this.$box.addClass('redactor-box-fullscreen');

    $('body, html').css('overflow', 'hidden');

    _this.fullscreen.resize();

    if (!_this.opts.fullscreen) {
      _this.selection.restore();
    }

    _this.toolbar.observeScrollDisable();

    $(window).on('resize.redactor-plugin-fullscreen', $.proxy(_this.fullscreen.resize, _this));
    ...
    setTimeout(() => {
      _this.fullscreen.isOpened = true;
    }, 10);
  },
...
const fonts = ['Arial', 'Helvetica', 'Georgia', 'Times New Roman', 'Monospace', 'Proxima', 'Alegreya', 'Lato', 'Lucida Sans Unicode', 'Merriweather', 'OpenSans', 'Palatino', 'Raleway', 'SourceSansPro'];
...
$.Redactor.prototype.fontcolor = () => ({
  init() {
    const buttons = ['fontcolor', 'backcolor'];

    for (let i = 0; i < 2; i += 1) {
      const name = buttons[i];
      const button = this.button.add(name, this.lang.get(name));
      const $dropdown = this.button.addDropdown(button);
  ...
  buildPicker($dropdown, name) {
    const rule = name === 'backcolor' ? 'background-color' : 'color';
    const func = e => {
      e.preventDefault();

      this.fontcolor.set($(this).data('rule'), $(this).attr('rel'));
    };

    colors.forEach(color => {
      const $swatch = $(`<a
        rel="${color}" data-rule="${rule}" href="#"
        style="
          float: left;
          font-size: 0;
    ...
    const $elNone = $('<a href="#" style="display: block; clear: both; padding: 5px; font-size: 12px; line-height: 1;"></a>').html(this.lang.get('none'));
    $elNone.on('click', e => {
      e.preventDefault();
...
const fonts$1 = [10, 11, 12, 14, 16, 18, 20, 24, 28, 30];
...
$$1.Redactor.prototype.limiter = () => ({
  init() {
    if (!this.opts.limiter) {
      return;
    }

    this.core.editor().on('keydown.redactor-plugin-limiter', $$1.proxy(e => {
      const key = e.which;
...
const buttons = ['html', 'formatting', 'bold', 'italic', 'underline', 'orderedlist', 'unorderedlist', 'outdent', 'indent', 'image', 'file', 'link', 'alignment', 'horizontalrule'];
